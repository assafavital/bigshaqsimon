...
public final class SimonClone {
	...
	public static final int TOTAL_BUTTONS = 4 * 4;
	...
	private boolean[] activeColors = new boolean [16];
	...
	public SimonClone(Context context) {
		...
		for (int i = 0; i < TOTAL_BUTTONS; ++i) {
			soundIds[i] = soundPool.load(context, R.raw.bigshaq, 1);
		}
...
//		soundIds[BLUE] = soundPool.load(context, R.raw.blue_long, 1);
	...
	public int getRandomColor () {
		int retval = RNG.nextInt(16);
		if (theGame == 3)  {			// Filter out inactive colors
			while (activeColors[retval] == false)  retval = RNG.nextInt(16);  // Keep trying till we get an active.
		}
		return retval;
	}
